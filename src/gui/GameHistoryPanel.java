...
public class GameHistoryPanel extends JPanel {
	...
	void redo(final Board board, 
        ...
        for(final Move move : moveHistory.getMoves()) {
        	...
        	} else if(move.getMovedPiece().getPieceAlliance().isWhite()) {
		...
		if(moveHistory.getMoves().size() > 0) {
			...
			if(lastMove.getMovedPiece().getPieceAlliance().isWhite()) {
				this.model.setValueAt(moveText + calculateCheckAndCheckMateHash(board), 
						currentRow -1, 1); // Adding symbols to history log when King is in check/check mate 
			}
		}
		final JScrollBar vertical = scrollPane.getVerticalScrollBar();
		vertical.setValue(vertical.getMaximum()); // Auto advancing to last move 
	}
	...
	// Swing evokes overridden models to render table 	
	@Override	
